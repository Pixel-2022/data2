...
video = cv2.VideoCapture('origin2/감사2_R.mp4')
...
for idx, action in enumerate(actions):
    ...
    result = np.array(all)
    np.save(os.path.join('dataset3', f'raw_{action}_{created_time}'), result)
    ...
    np.save(os.path.join('dataset3', f'seq_{action}_{created_time}'), full_seq_data)
